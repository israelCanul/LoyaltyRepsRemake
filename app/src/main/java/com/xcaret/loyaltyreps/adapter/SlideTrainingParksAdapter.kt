...
import com.xcaret.loyaltyreps.databinding.ParkItemBinding
...
class SlideInfoParksAdapter(
    val clickListener: (XPark) -> Unit,
    private val context: Context,
    val listItems:List<XPark>
): RecyclerView.Adapter<SlideInfoParksAdapter.InfoParksHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): InfoParksHolder {
        val itemBinding = ParkItemBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return InfoParksHolder(itemBinding)
    }
    override fun getItemCount() = listItems.size
    override fun onBindViewHolder(holder: InfoParksHolder, position: Int) {
        var park = listItems[position]
        holder.bind(context, park)
        holder.itemBinding.containerParkTraining.setOnClickListener {
            clickListener(park)
    ...
    inner class InfoParksHolder(val itemBinding: ParkItemBinding): RecyclerView.ViewHolder(itemBinding.root){
        fun bind(context: Context, xpark: XPark){
            val bitmap = getParkBitmapFromDrawableByName(context, xpark.name)
            bitmap?.let{
                itemBinding.imgTrainingItem.setImage(it)
